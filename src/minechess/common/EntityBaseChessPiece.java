...
public abstract class EntityBaseChessPiece extends EntityLiving{
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, float par2){
        if(MineChess.DEBUG || par1DamageSource.getDamageType() == "outOfWorld") {
        ...
        if(player.inventory.getCurrentItem() != null && player.inventory.getCurrentItem().itemID == MineChess.itemPieceMover.itemID && player.inventory.getCurrentItem().getItemDamage() < 2) {
            ...
            if(entitySelected != null && isBlack() ^ entitySelected.isBlack()) {
                ...
                if(!worldObj.isRemote && this instanceof EntityKing && player.isSneaking()) {
                    if(resignConfirmed) {
                        ...
                        MineChessUtils.sendUnlocalizedMessage(player, "message.player.youSurrender" + (isBlack() ? "WhiteWon" : "BlackWon"), EnumChatFormatting.RED.toString());
            ...
            if(!computerPiece) {
                ...
                if(enemy != null && enemy.mateInTimes < 0) {
                    ...
                    if(!worldObj.isRemote) MineChessUtils.sendUnlocalizedMessage(player, "message.player.transform" + (isBlack() ? "BlackAI" : "WhiteAI"), EnumChatFormatting.DARK_BLUE.toString());
                } else {
                    if(!worldObj.isRemote) MineChessUtils.sendUnlocalizedMessage(player, "message.error.aiPuzzle", EnumChatFormatting.RED.toString());
                }
            } else {
                if(!worldObj.isRemote) MineChessUtils.sendUnlocalizedMessage(player, "message.error.alreadyAI", EnumChatFormatting.RED.toString());
            }
        } else {
            if(!worldObj.isRemote) MineChessUtils.sendUnlocalizedMessage(player, "message.error.noPieceMover", EnumChatFormatting.RED.toString());
        }
    ...
    private void updateClient(List<int[]> validMoves, EntityPlayer player, int entityID){
        //  PacketDispatcher.sendPacketToPlayer(PacketHandler.getPieceSelectedUpdatePacket(validMoves, targetY - 1, entityID), (Player)player);
        ItemPieceMover.setEntitySelected(entityID, player.getCurrentEquippedItem());
        ItemPieceMover.setRenderTiles(validMoves, targetY - 1, player.getCurrentEquippedItem());
    }
    ...
    public boolean tryToGoTo(int x, int z, EntityPlayer player){
        ...
        if(isPawnAwaitingPromotion()) {
            if(player != null) MineChessUtils.sendUnlocalizedMessage(player, "message.error.pawnAwaitingPromotion", EnumChatFormatting.RED.toString());
        } else if(isBlack() == isBlackTurn && !isPieceCapturing()) {
            ...
            if(player != null) MineChessUtils.sendUnlocalizedMessage(player, "message.error." + (isBlackTurn ? "blacksTurn" : "whitesTurn"), EnumChatFormatting.RED.toString());
        }
        return false;
    }
    ...
    protected boolean isPawnAwaitingPromotion(){
        List<EntityBaseChessPiece> pieces = getChessPieces(true);
        for(EntityBaseChessPiece piece : pieces) {
            if(!piece.isDead && piece instanceof EntityPawn && piece.targetZ % 7 == 0) return true;
        }
        return false;
    }
                ...
                if(enemy.isBlack() == pieces.get(i).isBlack()) {
                    ...
                    if(moves.size() > 0) {
                        if(mateInTimes == 1) {
                            ...
                            setPuzzleFail();
                            MineChessUtils.onPuzzleFail(worldObj, player, this, xOffset, targetY, zOffset, rand);
                        }
                        mateInTimes--;
    ...
    public boolean canGoTo(int x, int z, boolean setCaptureIfneccessary, EntityPlayer player){
        ...
        if(player != null) MineChessUtils.sendUnlocalizedMessage(player, "message.error.movementRuleBlocks", EnumChatFormatting.RED.toString());
                    ...
                    for(int i = 0; i < chessPieces.size(); i++) {
                        if(chessPieces.get(i).getTargetPosition()[0] == targetX && chessPieces.get(i).getTargetPosition()[1] == targetZ && chessPieces.get(i) != this) {
                            if(chessPieces.get(i) instanceof EntityKing) {
                                ...
                                if(playerLoser != null) MineChessUtils.sendUnlocalizedMessage(playerLoser, "message.error.bugReport.capturedKing", EnumChatFormatting.RED.toString());
                            }
